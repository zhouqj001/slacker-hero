/**
 * Standalone tsdown config for the slacker-zone plugin.
 *
...
const PLUGIN_ID = '@slacker/ui-slacker'

const HERE = dirname(fileURLToPath(import.meta.url))

const CSS_VIRTUAL_PREFIX = '\0slacker-css:'
 ...
 * multi-entry build would emit ("locales-*.cjs") misses the table and the
...
const tea = browserClient({
  name: `${PLUGIN_ID}/tea`,
  entry: { tea: 'src/client/main-tea.tsx' },
})

const stockMini = browserClient({
  name: `${PLUGIN_ID}/stock-mini`,
  entry: { 'stock-mini': 'src/client/main-stock-mini.tsx' },
})
